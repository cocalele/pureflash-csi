...
func NewPfbdDriver(driverName, nodeID, endpoint string, version string) (*Lvm, error) {
	if driverName == "" {
...
func createProvisionerPod(ctx context.Context, va volumeAction) (err error) {
	...
	if va.action == actionTypeCreate && va.lvmType == "" {
		return fmt.Errorf("createlv without controller type")
	}
			...
			Containers: []v1.Container{
				{
					...
					VolumeMounts: []v1.VolumeMount{
						...
						},
						{
							Name:             "lvmbackup",
							ReadOnly:         false,
							MountPath:        "/etc/controller/backup",
							MountPropagation: &mountPropagationBidirectional,
						},
						{
							Name:             "lvmcache",
							ReadOnly:         false,
							MountPath:        "/etc/controller/cache",
							MountPropagation: &mountPropagationBidirectional,
						},
						{
							Name:             "lvmlock",
							ReadOnly:         false,
							MountPath:        "/run/lock/controller",
							MountPropagation: &mountPropagationBidirectional,
						},
					},
...
func CreateVG(name string, devicesPattern string) (string, error) {
	...
	tags := []string{"vg.metal-stack.io/csi-controller-driver"}
...
func CreateLVS(vg string, name string, size uint64, lvmType string) (string, error) {
	...
	tags := []string{"lv.metal-stack.io/csi-controller-driver"}
	for _, tag := range tags {
		args = append(args, "--addtag", tag)
	}
	args = append(args, vg)
